# Implementation Plan: WAF Statistics / Status Module

## Requirements Summary

Add a lock-free statistics and status subsystem to the existing `ngx_http_waf` module
(which ships one `.so` containing both `ngx_http_waf_module` (HTTP) and
`ngx_stream_waf_module` (STREAM)). The subsystem must:

1. Maintain in-process, cross-worker counters of every WAF verdict, the UA
...
- [ ] `GET /waf/stat/plain` returns a stub_status-style plaintext block including WAF
...
- [ ] `nginx -t` passes; the `.so` loads; no regression in existing WAF behaviour.
...
- STREAM verdict counters (global block) feeding the shared per-country table when geo
  resolved.
- Health/meta fields: `waf_up`, start time (master cycle-init epoch) / uptime, `last_reload_time`, geo DB
...
- New variables `$waf_country`, `$waf_reason`; extension of `ngx_http_waf_ctx_t`.
- Introduction of `ngx_http_waf_reason_e` enum + string tables as the single verdict
...
- Build glue: register `src/waf_status.c` in the addon `config`.
...
**Assumptions:**
- The WAF `.so` is loaded as a dynamic module and nginx is built `--with-stream`
...
live tree at `/mnt/nvme/imaginarium/openresty/modules/ngx_http_waf/`):

- **No shared state today.** `ngx_http_waf.h:57-61` explicitly documents that the
...
- **Only loc conf exists.** `ngx_http_waf_module_ctx` (`ngx_http_waf_module.c:196-208`)
...
- **All HTTP verdicts funnel through one handler.**
  `ngx_http_waf_preaccess_handler` (`ngx_http_waf_module.c:286-347`) is the single point
  where every HTTP verdict is decided and returned. Counter increments belong here.
- **All L4 verdicts funnel through one handler.** `ngx_stream_waf_handler`
  (`waf_stream.c:142-165`) — two outcomes only (`NGX_DECLINED` / `NGX_STREAM_FORBIDDEN`).
- **`reputation_check` is the shared verdict core** (`waf_reputation.c:14-98`), called
  by HTTP (`:316`), STREAM (`waf_stream.c:155`), and SMTP-auth (`waf_authhttp.c:65`). It
...
- **The 403 verdict is overloaded** — blocklist, network-flag, and geo-country all
  ...
  single source of truth (mirroring the existing `waf_type_str[]` table at
  `ngx_http_waf_module.c:26-33`).
- **Variable getter pattern is clean and copyable** — `$waf_type` getter at
  `ngx_http_waf_module.c:698-730`, registered in `ngx_http_waf_preconfiguration`
  (`:681-695`). New variables mirror this exactly.
- **Build is `config`-only.** Sources are a shell here-string in
  `modules/ngx_http_waf/config:22-28`; adding `src/waf_status.c` is a one-line edit +
  reconfigure. `CMakeLists.txt` already passes `--add-dynamic-module=` and provides the
  `waf_module` fast-rebuild target (`CMakeLists.txt:105-110`).
...
|---------|-----------|----------|
| reason enum + verdict type | `modules/ngx_http_waf/src/waf_rep.h` | core-only header (`:43` proto region) |
| reason string table `waf_reason_str[]` | `modules/ngx_http_waf/src/ngx_http_waf_module.c` | next to `waf_type_str[]` `:26-33` |
| ctx struct extension | `modules/ngx_http_waf/src/ngx_http_waf.h` | inside `ngx_http_waf_ctx_t` `:89-99` |
| shm counter struct defs | `modules/ngx_http_waf/src/waf_status.h` (NEW) | new file |
| main/srv conf structs | `modules/ngx_http_waf/src/ngx_http_waf.h` | new structs near loc conf `:64-81` |
| `reputation_check` out-param | `modules/ngx_http_waf/src/waf_reputation.c` | signature `:14`, body `:14-98` |
| `reputation_check` decl | `modules/ngx_http_waf/src/waf_rep.h` | `:43` |
| HTTP counter increments | `modules/ngx_http_waf/src/ngx_http_waf_module.c` | in `preaccess_handler` `:286-347` |
| STREAM counter increments | `modules/ngx_http_waf/src/waf_stream.c` | in `ngx_stream_waf_handler` `:142-165` |
| new variables registration | `modules/ngx_http_waf/src/ngx_http_waf_module.c` | in `preconfiguration` `:681-695` |
| new variable getters | `modules/ngx_http_waf/src/ngx_http_waf_module.c` | after `$waf_type` getter `:730` |
| module ctx conf slots | `modules/ngx_http_waf/src/ngx_http_waf_module.c` | `ngx_http_waf_module_ctx` `:196-208` |
| shm zone creation | `modules/ngx_http_waf/src/ngx_http_waf_module.c` | `postconfiguration` `:733-761` or new `init_main_conf` |
| per-vhost index assignment | `modules/ngx_http_waf/src/ngx_http_waf_module.c` | `postconfiguration` `:733-761` |
| `waf_status` command entry | `modules/ngx_http_waf/src/ngx_http_waf_module.c` | `ngx_http_waf_commands[]` `:70-193` |
| status handler + serializers | `modules/ngx_http_waf/src/waf_status.c` (NEW) | new file |
| build registration | `modules/ngx_http_waf/config` | `waf_srcs` `:22-28`, `waf_deps` `:14-20` |

### Current `ngx_http_waf_ctx_t` (to extend) — `ngx_http_waf.h:89-99`
```c
typedef struct {
...
### `$waf_type` getter to mirror — `ngx_http_waf_module.c:698-730`
```c
static ngx_int_t
...
```
Registration — `ngx_http_waf_module.c:681-695`:
```c
static ngx_int_t
...
### `ngx_http_waf_preaccess_handler` (verdict decision points) — `ngx_http_waf_module.c:286-347`
```c
static ngx_int_t
...
### `ngx_http_waf_reputation_check` (to extend with geo/reason out) — `waf_reputation.c:14-98`
Current signature (`waf_rep.h:43`):
```c
ngx_int_t ngx_http_waf_reputation_check(ngx_waf_rep_conf_t *rep,
    struct sockaddr *sa, ngx_str_t *reason);
```
Key internals: geo lookup at `waf_reputation.c:46` into local `res`; packed code
...
NULL where they do not need it). The verdict type and reason enum live in **`waf_rep.h`**
(ngx_core-only), so they must NOT embed `ngx_http_waf_geo_result_t` by value (that type
lives in `waf_geo.h`, which the stream TU does not include). Store only the primitive
geo fields needed:
```c
/* in waf_rep.h, alongside the prototype */
typedef enum { /* ... */ } ngx_http_waf_reason_e;

typedef struct {
    ngx_http_waf_reason_e  reason;        /* WAF_REASON_* token, set on every return */
...
ngx_int_t ngx_http_waf_reputation_check(ngx_waf_rep_conf_t *rep,
    struct sockaddr *sa, ngx_str_t *reason, ngx_http_waf_verdict_t *out /* may be NULL */);
```
...
`waf_reason_str[out->reason]`).

### Geo result type + lookup API — `waf_geo.h:53-70`
```c
typedef struct {
...
### `ngx_stream_waf_handler` (L4 verdicts) — `waf_stream.c:142-165`
```c
static ngx_int_t
ngx_stream_waf_handler(ngx_stream_session_t *s)
{
    ...
    rc = ngx_http_waf_reputation_check(&sscf->rep, s->connection->sockaddr, &reason);
    if (rc != NGX_DECLINED) {
...
### Module ctx conf slots (to fill main + srv) — `ngx_http_waf_module.c:196-208`
```c
...
### postconfiguration (phase handler registration) — `ngx_http_waf_module.c:733-761`
```c
static ngx_int_t
ngx_http_waf_postconfiguration(ngx_conf_t *cf)
{
    ...
    *h = ngx_http_waf_preaccess_handler;
    if (ngx_http_waf_spoof_init(cf) != NGX_OK) { return NGX_ERROR; }
    return NGX_OK;
}
```
...
### `waf_status` command entry (template) — add to `ngx_http_waf_commands[]` `:70-193`
Follow the `waf_mail_auth` NOARGS content-handler pattern (`:165-170`):
```c
    { ngx_string("waf_status"),
      NGX_HTTP_LOC_CONF|NGX_CONF_NOARGS,
      ngx_http_waf_set_status,        /* sets clcf->handler = ngx_http_waf_status_handler */
...
### Build registration — `modules/ngx_http_waf/config:22-28`
```sh
waf_srcs="$ngx_addon_dir/src/ngx_http_waf_module.c \
         $ngx_addon_dir/src/waf_match.c \
         $ngx_addon_dir/src/waf_spoof.c \
         $ngx_addon_dir/src/waf_geo.c \
         $ngx_addon_dir/src/waf_reputation.c \
         $ngx_addon_dir/src/waf_authhttp.c \
         $ngx_addon_dir/src/waf_stream.c"
```
Append `\\\n         $ngx_addon_dir/src/waf_status.c` to `waf_srcs`; add
`$ngx_addon_dir/src/waf_status.h` to `waf_deps` (`:14-20`).
...
- **(b) `ngx_http_waf_reason_e` enum as single source** feeding counter index,
...
**`ngx_http_waf_reason_e`** (new, in `waf_rep.h` — the ngx_core-only header, NOT
`ngx_http_waf.h`, so the stream TU which includes only `waf_rep.h` can name it):
```c
typedef enum {
    WAF_REASON_NONE = 0,      /* allowed (no rule matched) */
    WAF_REASON_ALLOWLIST,     /* allowlist short-circuit   */
    WAF_REASON_BLOCKLIST,     /* static CIDR -> 403        */
    WAF_REASON_GEO,           /* geo country block -> 403  */
    WAF_REASON_GEO_WHITELIST, /* not whitelisted -> 404    */
    WAF_REASON_FLAG,          /* network flag -> 403       */
    WAF_REASON_SCANNER_UA,    /* scanner UA -> 404         */
    WAF_REASON_EMPTY_UA,      /* empty UA -> 404           */
    WAF_REASON_SCANNER_PATH,  /* scanner path -> 404/403/444 */
    WAF_REASON_MAX
} ngx_http_waf_reason_e;
```
Parallel `static ngx_str_t waf_reason_str[WAF_REASON_MAX]` with tokens:
`none, allowlist, blocklist, geo, geo_whitelist, flag, scanner_ua, empty_ua, scanner_path`.

**Shm counter struct** (new, in `waf_status.h`):
```c
#define WAF_STAT_CC_SLOTS   512   /* closed-set country table, open-addressed */
#define WAF_FLAG_SLOTS      4     /* anon, satellite, anycast, drop            */
/* NOTE: Tor has NO libloc flag bit (waf_geo.h:33-36 defines only 4 bits); Tor blocks
 * surface as a CC "T1" geo-country verdict (WAF_REASON_GEO), NOT a flag verdict. The
...
typedef struct {
    ...
    ngx_atomic_t  http_blocked[WAF_REASON_MAX]; /* per-reason block counts */
    ...
    ngx_atomic_t  http_ua[WAF_UA_MAX];          /* UA classification dist  */
    ngx_atomic_t  flag_blocked[WAF_FLAG_SLOTS]; /* per-flag breakdown      */
...
- `reputation_check` gains a trailing `out` param (NULL = caller wants no verdict
  detail). The prototype lives in `waf_rep.h:43` (`waf_reputation.h` is a thin shim with
  NO prototype — do not edit it). All THREE call sites are updated in the same change:
  HTTP `ngx_http_waf_module.c:316`, STREAM `waf_stream.c:155`, SMTP-auth
  `waf_authhttp.c:65`. HTTP AND STREAM pass a real `ngx_http_waf_verdict_t *out` (HTTP
...
1. **Reason enum + string tables**: add `ngx_http_waf_reason_e` AND
   `ngx_http_waf_verdict_t` to **`waf_rep.h`** (the ngx_core-only header included by the
   stream TU — NOT `ngx_http_waf.h`, which pulls `ngx_http.h`). Add `waf_reason_str[]` in
   `ngx_http_waf_module.c`. Keep the existing human `reason` strings in `reputation_check`
   or derive them from a second parallel table.
2. **Extend ctx** (`ngx_http_waf.h:89-99`): add `geo_done:1`, `verdict_set:1`,
   `country[2]`, `reason`.
3. **Extend `reputation_check`** (declaration `waf_rep.h:43`; body `waf_reputation.c:14-98`):
   add the `ngx_http_waf_verdict_t *out` param; set `out->reason` on EVERY branch
   (including both allow paths: `WAF_REASON_ALLOWLIST` for the allowlist short-circuit at
   `:30`, `WAF_REASON_NONE` for the geo-pass fall-through); copy `res` into `out->geo`
   ...
   `ctx->geo_done` when `out->geo_valid`), STREAM `waf_stream.c:155` (passes `out`; reads
   `out->reason` for `stream_denied[]`), SMTP-auth `waf_authhttp.c:65` (passes NULL).
   Do NOT touch `waf_reputation.h` (shim, no prototype). **NULL-guard:** every `out->...`
...
4. **Shm struct + main conf + init/re-attach** (`waf_status.h`, `ngx_http_waf_module.c`):
   ...
   `ngx_shared_memory_add(cf, &name, size, &ngx_http_waf_module)` and store the returned
   `ngx_shm_zone_t *` in main conf. Pass a stable **`tag`** (`zone->tag =
   &ngx_http_waf_module`) so the SAME name+tag re-add from the STREAM module resolves to
   ...
   **STREAM zone access:** the STREAM module is a distinct `ngx_stream_module_t` and
   ...
   `ngx_shared_memory_add` with the identical name + `tag = &ngx_http_waf_module` from its
...
5. **Srv conf + per-vhost index** (`ngx_http_waf_module.c`): add
   `create_srv_conf`/`merge_srv_conf`; in `postconfiguration` walk `cmcf->servers` (each
   `ngx_http_core_srv_conf_t` exposes `->ctx->srv_conf[ngx_http_waf_module.ctx_index]`),
   assign each WAF srv conf a sequential `stat_index`, store `nvhosts` in main conf. The
...
6. **Counter increments.** In `preaccess_handler` (`:286-347`), AFTER the early guards
   ...
   `wmcf = ngx_http_get_module_main_conf(r, ngx_http_waf_module)` (-> shm `sh = wmcf->stat_zone->data`)
   and `wscf = ngx_http_get_module_srv_conf(r, ngx_http_waf_module)` (-> `wscf->stat_index`,
   ...
   `ngx_http_waf_stat_cc_bump(sh, cc16, blocked?)`. `WAF_REASON_ALLOWLIST` vs
   `WAF_REASON_NONE` both map to `http_allowed` but are distinguished for `$waf_reason`.
   In `ngx_stream_waf_handler` (`waf_stream.c:142-165`) the handler reads the shm via the
   ...
   **`cc_bump` claim protocol:** open-addressed probe on `cc16`; an empty slot has
   ...
   unreachable — invariant guaranteed by `waf_reputation.c:67`). Claim an empty slot with
   `ngx_atomic_cmp_set(&slot->cc16, 0, key)`; on CAS failure re-read the slot key and, if
   it now equals `key`, use it, else probe the next slot. If all `WAF_STAT_CC_SLOTS` are
   taken, bump a `cc_overflow` counter and skip (defensive; 512 >> ~250).
7. **New variables** (`ngx_http_waf_module.c`): register `$waf_country` and
   `$waf_reason` in `preconfiguration` (`:681-695`); add getters after `:730`.
   `$waf_country`: if `!geo_done && wlcf->rep.geo_db != NULL`, perform a lazy
   `ngx_http_waf_geo_lookup` and cache into `ctx->country`/`geo_done`; emit 2 bytes or
   `not_found`. `$waf_reason`: emit `waf_reason_str[ctx->reason]` (or `none` if unset).
8. **`waf_status` directive + content handler** (`ngx_http_waf_commands[]` `:70-193`,
   new `waf_status.c`): `ngx_http_waf_set_status` sets `clcf->handler =
   ngx_http_waf_status_handler`. The handler restricts to GET/HEAD, takes a snapshot of
   the shm struct, parses the last URI path segment, and dispatches to the serializer.
9. **Three serializers** (`waf_status.c`): `plain` (stub-style table; default for unknown
   segment), `json` (nested object), `prometheus` (text-exposition; metrics
   `waf_http_requests_total`, `waf_http_blocked_total{reason=...}`,
   `waf_http_blocked_total{reason="flag",flag=...}`, `waf_blocked_by_country_total{country=...,blocked|total}`,
...
10. **Build glue** (`modules/ngx_http_waf/config:14-28`): add `src/waf_status.c` to
    `waf_srcs` and `src/waf_status.h` to `waf_deps`; reconfigure (touch the OpenSSL
...
- **Per-country table full** (all `WAF_STAT_CC_SLOTS` claimed) → drop the increment for
...
- Geo DB staleness: `time() - waf_geo_db_created_seconds` over threshold.
- Block-rate spikes per country/vhost via `rate(waf_http_blocked_total[5m])`.

### Debugging
- Plaintext endpoint doubles as the human debug view; include `waf_up`, uptime,
...
### Code Documentation
- [ ] Doxygen-style comments on `waf_status.c` public functions and the shm struct.
...
### New Documentation
- [ ] A short "WAF metrics reference" listing every metric, its labels, and meaning.
...
- `modules/ngx_http_waf/src/ngx_http_waf.h` — reason enum, ctx extension, main/srv conf
  structs.
- `modules/ngx_http_waf/src/ngx_http_waf_module.c` — conf slots, postconfiguration
  (shm + vhost index), command entry, variable registration + getters, counter
  increments in the preaccess handler.
- `modules/ngx_http_waf/src/waf_reputation.c` + `waf_rep.h` — `reputation_check` out-param.
- `modules/ngx_http_waf/src/waf_stream.c` — STREAM counter increments + call-site update.
- `modules/ngx_http_waf/src/waf_authhttp.c` — SMTP-auth `reputation_check` call-site update.
- `modules/ngx_http_waf/src/waf_status.c` (NEW) — shm struct, status handler, 3 serializers.
- `modules/ngx_http_waf/src/waf_status.h` (NEW) — shm struct + public API.
- `modules/ngx_http_waf/config` — register the new source/header.
...
- [ ] Existing WAF behaviour unregressed (`$waf_type`, all verdicts, status codes)
