/*
 * ngx_http_heavybag_module - JA4 TLS client fingerprint (FoxIO JA4, observability)
 *
 * Two layers:
 *   ngx_http_heavybag_ja4_build()   - PURE core. Takes raw ClientHello field arrays
 ...
 *                                standalone under -DHEAVYBAG_JA4_UNIT_TEST.
 *   ngx_http_heavybag_ja4_compute() - thin SSL extractor (task-014) that reads the
 ...
 * list is GREASE-filtered and clamped to HEAVYBAG_JA4_MAX_ELEMS before any copy or
 * sort (CWE-190/787); the output is a fixed HEAVYBAG_JA4_LEN buffer written only
...
#ifndef _HEAVYBAG_JA4_H_INCLUDED_
#define _HEAVYBAG_JA4_H_INCLUDED_
...
#define HEAVYBAG_JA4_LEN        37

/* hard clamp on any parsed uint16 list before copy/sort (CWE-190/787) */
#define HEAVYBAG_JA4_MAX_ELEMS  256


/*
 * Build a FoxIO JA4 fingerprint into out[HEAVYBAG_JA4_LEN] (always NUL-terminated
 ...
 */
int ngx_http_heavybag_ja4_build(const uint16_t *ciphers, size_t n_ciphers,
 ...
 * Defined in heavybag_ja4.c (task-014).
 */
ngx_int_t ngx_http_heavybag_ja4_compute(struct ssl_st *ssl, ngx_pool_t *pool,
    ngx_str_t *out);

#endif /* HEAVYBAG_JA4_UNIT_TEST */


#endif /* _HEAVYBAG_JA4_H_INCLUDED_ */
