 ...
 *
 * Compiled standalone: includes waf_ua_parse.c under -DWAF_UA_PARSE_UNIT_TEST
 * so only the nginx-free core is pulled in (no nginx / SSL headers). Built +
 * run by run-unit-tests.sh with `cc -DWAF_UA_PARSE_UNIT_TEST` (no libcrypto --
...
#define WAF_UA_PARSE_UNIT_TEST
#endif
#include "../../src/waf_ua_parse.c"
...
typedef struct {
    ...
    ngx_http_waf_ua_category_e  c;
    ngx_http_waf_ua_vendor_e    v;
    const u_char               *vs;
    size_t                      vl;
} pr_t;
...
{
    pr_t  r;
    waf_ua_parse_core((const u_char *) ua, strlen(ua),
...
CTEST(ua_browser, chrome_desktop)
{
    ...
    ASSERT_EQUAL(WAF_BROWSER_CHROME, r.b);
    ASSERT_EQUAL(WAF_OS_WINDOWS, r.o);
    ASSERT_TRUE(veq(r, "120.0.0.0"));
    ASSERT_EQUAL(WAF_VENDOR_GOOGLE, r.v);
}
...
CTEST(ua_browser, firefox)
{
    pr_t r = P("Mozilla/5.0 (X11; Linux x86_64; rv:124.0) Gecko/20100101 "
               "Firefox/124.0");
    ASSERT_EQUAL(WAF_BROWSER_FIREFOX, r.b);
    ASSERT_EQUAL(WAF_OS_LINUX, r.o);
    ASSERT_TRUE(veq(r, "124.0"));
    ASSERT_EQUAL(WAF_VENDOR_MOZILLA, r.v);
}
...
CTEST(ua_browser, safari_macos)
{
    pr_t r = P("Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7) "
               ...
               "Safari/605.1.15");
    ASSERT_EQUAL(WAF_BROWSER_SAFARI, r.b);
    ASSERT_EQUAL(WAF_OS_MACOS, r.o);
    ASSERT_EQUAL(WAF_VENDOR_APPLE, r.v);
}
...
CTEST(ua_browser, edge_over_chrome)
{
    ...
    ASSERT_EQUAL(WAF_BROWSER_EDGE, r.b);
    ASSERT_TRUE(veq(r, "120.0.2210.91"));
    ASSERT_EQUAL(WAF_VENDOR_MS, r.v);
}
...
CTEST(ua_browser, opera_over_chrome)
{
    ...
    ASSERT_EQUAL(WAF_BROWSER_OPERA, r.b);
    ASSERT_EQUAL(WAF_VENDOR_OPERA, r.v);
}
...
CTEST(ua_browser, operagx_over_opr)
{
    ...
    ASSERT_EQUAL(WAF_BROWSER_OPERAGX, r.b);
    ASSERT_EQUAL(WAF_VENDOR_OPERA, r.v);
}
...
CTEST(ua_browser, yabrowser_over_chrome)
{
    ...
    ASSERT_EQUAL(WAF_BROWSER_YABROWSER, r.b);
    ASSERT_EQUAL(WAF_VENDOR_YANDEX, r.v);
}
...
CTEST(ua_browser, samsung_over_chrome)
{
    pr_t r = P("Mozilla/5.0 (Linux; Android 13) AppleWebKit/537.36 (KHTML, "
               ...
               "Safari/537.36");
    ASSERT_EQUAL(WAF_BROWSER_SAMSUNG, r.b);
    ASSERT_EQUAL(WAF_VENDOR_SAMSUNG, r.v);
}
...
CTEST(ua_browser, headlesschrome_over_chrome)
{
    ...
    ASSERT_EQUAL(WAF_BROWSER_HEADLESSCHROME, r.b);
    ASSERT_TRUE(veq(r, "120.0.0.0"));
    ASSERT_EQUAL(WAF_VENDOR_GOOGLE, r.v);
}
...
CTEST(ua_browser, duckduckgo_over_chrome)
{
    ...
    ASSERT_EQUAL(WAF_BROWSER_DUCKDUCKGO, r.b);
    ASSERT_EQUAL(WAF_VENDOR_DUCKDUCKGO, r.v);
}

CTEST(ua_browser, curl_tool)
{
    pr_t r = P("curl/8.5.0");
    ASSERT_EQUAL(WAF_BROWSER_CURL, r.b);
...
CTEST(ua_browser, python_requests)
{
    ...
    ASSERT_EQUAL(WAF_BROWSER_PYTHON, r.b);
...
CTEST(ua_browser, go_http_client)
{
    ...
    ASSERT_EQUAL(WAF_BROWSER_GOHTTP, r.b);
}

CTEST(ua_browser, okhttp)
{
    pr_t r = P("okhttp/4.12.0");
    ASSERT_EQUAL(WAF_BROWSER_OKHTTP, r.b);
}

CTEST(ua_browser, empty_is_unknown)
{
    pr_t r = P("");
    ASSERT_EQUAL(WAF_BROWSER_UNKNOWN, r.b);
...
CTEST(ua_os, iphone)
{
    ...
    ASSERT_EQUAL(WAF_OS_IPHONE, r.o);
    ASSERT_EQUAL(WAF_CAT_MOBILE, r.c);
}
...
CTEST(ua_os, android)
{
    ...
    ASSERT_EQUAL(WAF_OS_ANDROID, r.o);
    ASSERT_EQUAL(WAF_CAT_MOBILE, r.c);
}
...
CTEST(ua_os, harmonyos_over_android)
{
    ...
    ASSERT_EQUAL(WAF_OS_HARMONYOS, r.o);
    ASSERT_EQUAL(WAF_BROWSER_HUAWEIBROWSER, r.b);
    ASSERT_EQUAL(WAF_VENDOR_HUAWEI, r.v);   /* HuaweiBrowser/ -> huawei */
}
...
CTEST(ua_os, windows_pc)
{
    ...
    ASSERT_EQUAL(WAF_OS_WINDOWS, r.o);
    ASSERT_EQUAL(WAF_CAT_PC, r.c);
}

CTEST(ua_os, console_ps4)
{
    pr_t r = P("Mozilla/5.0 (PlayStation 4 8.50) AppleWebKit/605.1.15");
    ASSERT_EQUAL(WAF_OS_PS4, r.o);
    ASSERT_EQUAL(WAF_CAT_CONSOLE, r.c);
}
...
CTEST(ua_vendor, baidu)
{
    ...
    ASSERT_EQUAL(WAF_VENDOR_BAIDU, r.v);
}
...
{
    ASSERT_EQUAL(WAF_VENDOR_APPLE,
        waf_ua_try_vendor((const u_char *) "x", 1,
                          WAF_OS_IPAD, WAF_BROWSER_SAFARI));
}
...
{
    ASSERT_EQUAL(WAF_TLSFAM_CHROMIUM,
                 ngx_http_waf_ua_expected_tls_family(WAF_BROWSER_CHROME));
    ASSERT_EQUAL(WAF_TLSFAM_CHROMIUM,
                 ngx_http_waf_ua_expected_tls_family(WAF_BROWSER_EDGE));
    ASSERT_EQUAL(WAF_TLSFAM_FIREFOX,
                 ngx_http_waf_ua_expected_tls_family(WAF_BROWSER_FIREFOX));
    ASSERT_EQUAL(WAF_TLSFAM_SAFARI,
                 ngx_http_waf_ua_expected_tls_family(WAF_BROWSER_SAFARI));
    ASSERT_EQUAL(WAF_TLSFAM_TOOL,
                 ngx_http_waf_ua_expected_tls_family(WAF_BROWSER_CURL));
    /* ambiguous / legacy browsers -> UNKNOWN (no false-positive spoof) */
    ASSERT_EQUAL(WAF_TLSFAM_UNKNOWN,
                 ngx_http_waf_ua_expected_tls_family(WAF_BROWSER_MSIE));
    ASSERT_EQUAL(WAF_TLSFAM_UNKNOWN,
                 ngx_http_waf_ua_expected_tls_family(WAF_BROWSER_UNKNOWN));
}
...
/* Ascending by the lexicographic+length order ngx_http_waf_ja4_entry_cmp
 * imposes: 'c' < 'f' < 's' < 't' < 'z'. */
static const ngx_http_waf_ja4_entry_t  ja4_tbl[] = {
    { { sizeof("chrome_fp") - 1,    (u_char *) "chrome_fp" },    WAF_TLSFAM_CHROMIUM },
    { { sizeof("firefox_fp") - 1,   (u_char *) "firefox_fp" },   WAF_TLSFAM_FIREFOX },
    { { sizeof("safari_fp") - 1,    (u_char *) "safari_fp" },    WAF_TLSFAM_SAFARI },
    { { sizeof("tool_fp") - 1,      (u_char *) "tool_fp" },      WAF_TLSFAM_TOOL },
    { { sizeof("zzz_ambiguous") - 1,(u_char *) "zzz_ambiguous" },WAF_TLSFAM_UNKNOWN },
};
...
lk(const char *s)
{
    return waf_ja4_family_lookup(ja4_tbl, JA4_N,
                                 (const u_char *) s, strlen(s));
}

CTEST(ja4_family, known_fingerprints)
{
    ASSERT_EQUAL(WAF_TLSFAM_CHROMIUM, lk("chrome_fp"));
    ASSERT_EQUAL(WAF_TLSFAM_FIREFOX,  lk("firefox_fp"));
    ASSERT_EQUAL(WAF_TLSFAM_SAFARI,   lk("safari_fp"));
    ASSERT_EQUAL(WAF_TLSFAM_TOOL,     lk("tool_fp"));
}

CTEST(ja4_family, ambiguous_is_unknown)
{
    ASSERT_EQUAL(WAF_TLSFAM_UNKNOWN, lk("zzz_ambiguous"));
}

CTEST(ja4_family, absent_is_unknown)
{
    ASSERT_EQUAL(WAF_TLSFAM_UNKNOWN, lk("absent_fp"));
    ASSERT_EQUAL(WAF_TLSFAM_UNKNOWN, lk("chrome_f"));   /* prefix, not exact */
    ASSERT_EQUAL(WAF_TLSFAM_UNKNOWN, lk("chrome_fpX")); /* longer, not exact */
}

CTEST(ja4_family, empty_table_or_key_is_unknown)
{
    ASSERT_EQUAL(WAF_TLSFAM_UNKNOWN,
                 waf_ja4_family_lookup(NULL, 0, (const u_char *) "x", 1));
    ASSERT_EQUAL(WAF_TLSFAM_UNKNOWN,
                 waf_ja4_family_lookup(ja4_tbl, JA4_N, (const u_char *) "", 0));
}
 ...
 * different => spoof. */
static int
ja4_signal(ngx_http_waf_tls_family_e fam_ja4, ngx_http_waf_ua_browser_e b)
{
    ngx_http_waf_tls_family_e  fam_ua = ngx_http_waf_ua_expected_tls_family(b);
    return fam_ja4 != WAF_TLSFAM_UNKNOWN && fam_ua != WAF_TLSFAM_UNKNOWN
           && fam_ja4 != fam_ua;
}
...
CTEST(spoof, tool_ja4_with_chrome_ua_is_spoof)
{
    ...
    ASSERT_TRUE(ja4_signal(WAF_TLSFAM_TOOL, WAF_BROWSER_CHROME));
}

CTEST(spoof, matching_family_is_not_spoof)
{
    ASSERT_FALSE(ja4_signal(WAF_TLSFAM_CHROMIUM, WAF_BROWSER_CHROME));
    ASSERT_FALSE(ja4_signal(WAF_TLSFAM_FIREFOX, WAF_BROWSER_FIREFOX));
}

CTEST(spoof, unknown_ja4_is_not_spoof)
{
    /* an unknown/absent JA4 never triggers (avoids false positives) */
    ASSERT_FALSE(ja4_signal(WAF_TLSFAM_UNKNOWN, WAF_BROWSER_CHROME));
}

CTEST(spoof, unmapped_ua_is_not_spoof)
{
    /* a UA browser with no stable TLS family (msie) never triggers */
    ASSERT_FALSE(ja4_signal(WAF_TLSFAM_CHROMIUM, WAF_BROWSER_MSIE));
}
