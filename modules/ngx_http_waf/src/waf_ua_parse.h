 ...
 * (ngx_http_waf_ua_classify). It produces five read-only nginx variables
 ...
 * -DWAF_UA_PARSE_UNIT_TEST; this header is the nginx-facing API and is NOT
 * included by the test.
 */

#ifndef _WAF_UA_PARSE_H_INCLUDED_
#define _WAF_UA_PARSE_H_INCLUDED_
 ...
 * calls ngx_http_waf_ua_classify() first, so ua_category can take over the
 ...
 * table (wlcf->ja4_table). Returns WAF_TLSFAM_UNKNOWN when the table is
 ...
 * @return coarse TLS family, or WAF_TLSFAM_UNKNOWN
 */
ngx_http_waf_tls_family_e ngx_http_waf_ja4_family(
    ngx_http_waf_loc_conf_t *wlcf, ngx_str_t *ja4);


/**
 * Map a parsed UA browser to the coarse TLS family it SHOULD present at the
 * TLS layer. Chromium variants all map to WAF_TLSFAM_CHROMIUM (JA4 cannot
 * split them). Returns WAF_TLSFAM_UNKNOWN for browsers with no stable TLS
 * family expectation.
 *
 * @param b parsed UA browser enum
 * @return expected coarse TLS family, or WAF_TLSFAM_UNKNOWN
 */
ngx_http_waf_tls_family_e ngx_http_waf_ua_expected_tls_family(
    ngx_http_waf_ua_browser_e b);


#endif /* _WAF_UA_PARSE_H_INCLUDED_ */
