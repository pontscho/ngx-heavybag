/*
 * ngx_http_waf_module - lock-free per-IP token-bucket rate limiter (core)
 *
 * Fixed-size, open-addressed per-IP table in one shared zone, modelled on the
 * cc[512] country table's lock-free claim (ngx_http_waf_stat_cc_bump) rather
...
#define WAF_RATE_PROBE_MAX   32      /* open-addressing probe window  */
#define WAF_RATE_CAS_MAX     16      /* bounded state-CAS retries     */
#define WAF_RATE_MIN_SLOTS   64      /* a smaller zone is a misconfig */

#define WAF_RATE_FNV_OFFSET  14695981039346656037ULL
#define WAF_RATE_FNV_PRIME   1099511628211ULL


typedef struct {
    ngx_atomic_t  key;     /* FNV-1a hash; 0 = empty                       */
    ngx_atomic_t  state;   /* (last_refill_ms32 << 32) | tokens_fp32       */
} ngx_http_waf_rate_slot_t;


typedef struct {
    ngx_atomic_t  rate_overflow;   /* eviction-CAS failures (saturation)   */
    ngx_uint_t    nslots;          /* length of the trailing slot array     */
    /* ngx_http_waf_rate_slot_t slots[nslots] follow, contiguous */
} ngx_http_waf_rate_hdr_t;
 ...
 */
static uint64_t
ngx_http_waf_rate_key(struct sockaddr *sa)
{
    u_char               *bytes;
    size_t                len, i;
    uint64_t              h;
    ...
    h = WAF_RATE_FNV_OFFSET;
    h ^= tag;
    h *= WAF_RATE_FNV_PRIME;
...
ngx_int_t
ngx_http_waf_rate_check(void *shm, struct sockaddr *sa,
    uint64_t rate_num_fp, ngx_uint_t period_ms, ngx_uint_t burst_fp)
{
    ...
    ngx_http_waf_rate_slot_t  *slots, *s, *oldest, *cand;
    ngx_http_waf_rate_hdr_t   *hdr = shm;

    if (hdr == NULL) {
        return NGX_OK;                       /* no zone -> fail-open */
    }

    h = ngx_http_waf_rate_key(sa);
    if (h == 0) {
    ...
    slots = (ngx_http_waf_rate_slot_t *) (hdr + 1);
    idx = (ngx_uint_t) (h % n);
    s = NULL;
    oldest = NULL;
    oldest_key = 0;
     ...
     */
    for (p = 0; p < WAF_RATE_PROBE_MAX; p++) {
        cand = &slots[(idx + p) % n];
...
ngx_http_waf_rate_rule_select(ngx_array_t *rules, ngx_http_waf_verdict_t *verdict)
{
    uint16_t                   cc16, *ccs;
    ngx_uint_t                 i, j;
    ngx_http_waf_rate_rule_t  *rule, *deflt;
...
char *
ngx_http_waf_rate_rule_add(ngx_conf_t *cf, ngx_array_t **rules)
{
    ...
    ngx_array_t               *geo_cc;
    ngx_http_waf_rate_rule_t  *rule, *r;
    ...
    for (i = 1; i < cf->args->nelts; i++) {
        ...
        if (a->len > 5 && ngx_strncmp(a->data, "rate=", 5) == 0) {
            ...
            if ((uint64_t) num > (0xffffffffULL / WAF_RATE_SCALE)) {
                return "rate is too large";
            }
            rate_num_fp = (uint64_t) num * WAF_RATE_SCALE;
            continue;
        }
        ...
        if (a->len > 6 && ngx_strncmp(a->data, "burst=", 6) == 0) {
            ...
            burst_fp = (uint64_t) num * WAF_RATE_SCALE;
            continue;
        }
        ...
        if (a->len > 8 && ngx_strncmp(a->data, "for_geo=", 8) == 0) {
            ...
            while (p < last) {
                ...
                cc.data = ds;
                cc.len = p - ds;
                if (ngx_http_waf_country_add(cf, &geo_cc, &cc) != NGX_OK) {
...
ngx_int_t
ngx_http_waf_rate_init_zone(ngx_shm_zone_t *shm_zone, void *data)
{
    size_t                    alloc, margin;
    ngx_uint_t                n, pages;
    ngx_slab_pool_t          *shpool;
    ngx_http_waf_rate_hdr_t  *hdr;
    ngx_http_waf_rate_hdr_t  *ohdr = data;
    ...
    if (shm_zone->shm.size <= margin
        || (shm_zone->shm.size - margin) / sizeof(ngx_http_waf_rate_slot_t)
           < WAF_RATE_MIN_SLOTS)
    {
        ngx_log_error(NGX_LOG_EMERG, shm_zone->shm.log, 0,
                      "waf: waf_rate zone is too small");
        return NGX_ERROR;
    }
    ...
    for ( ;; ) {
        if (n < WAF_RATE_MIN_SLOTS) {
            ngx_log_error(NGX_LOG_EMERG, shm_zone->shm.log, 0,
                          "waf: waf_rate zone is too small");
            return NGX_ERROR;
        }

        alloc = sizeof(ngx_http_waf_rate_hdr_t)
                + (size_t) n * sizeof(ngx_http_waf_rate_slot_t);
...
ngx_atomic_uint_t
ngx_http_waf_rate_overflow(void *shm)
{
    ngx_http_waf_rate_hdr_t  *hdr = shm;
