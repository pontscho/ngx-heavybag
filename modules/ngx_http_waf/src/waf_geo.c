 ...
 */
#define NGX_HTTP_WAF_GEO_HDR_SIZE   4192   /* on-disk header size after magic  */
#define NGX_HTTP_WAF_GEO_DATA_OFF   (8 + NGX_HTTP_WAF_GEO_HDR_SIZE)  /* 4200    */
#define NGX_HTTP_WAF_GEO_SIG_MAX    2048   /* per-slot signature blob capacity */
#define NGX_HTTP_WAF_GEO_ZERO_BEG   60     /* copy-local: first u16 length     */
#define NGX_HTTP_WAF_GEO_ZERO_END   4160   /* copy-local: end of sig2 blob     */
/* file-absolute signature offsets (base + 8 + copy-local): */
#define NGX_HTTP_WAF_GEO_SIG1_LEN   (8 + 60)    /* 68   */
#define NGX_HTTP_WAF_GEO_SIG2_LEN   (8 + 62)    /* 70   */
#define NGX_HTTP_WAF_GEO_SIG1_OFF   (8 + 64)    /* 72   */
#define NGX_HTTP_WAF_GEO_SIG2_OFF   (8 + 2112)  /* 2120 */
 ...
 */
#define NGX_HTTP_WAF_GEO_MAX_SIZE   (512UL * 1024 * 1024)
 ...
 */
static const char  ngx_http_waf_geo_signing_key[] =
"-----BEGIN PUBLIC KEY-----\n"
...
ngx_http_waf_geo_u16(const u_char *p)
{
 ...
 * OpenSSL (libcrypto is already linked, see waf_ja4.c; no libloc dependency).
 *
 ...
 *
 * The caller guarantees size >= NGX_HTTP_WAF_GEO_DATA_OFF (pre-mmap guard).
 */
static ngx_int_t
ngx_http_waf_geo_verify(ngx_conf_t *cf, ngx_str_t *full,
    const u_char *base, size_t size)
{
    ...
    unsigned long   e;
    u_char          hdr[NGX_HTTP_WAF_GEO_HDR_SIZE];
    char            errbuf[256];

    sig1_len = ngx_http_waf_geo_u16(base + NGX_HTTP_WAF_GEO_SIG1_LEN);
    sig2_len = ngx_http_waf_geo_u16(base + NGX_HTTP_WAF_GEO_SIG2_LEN);

    /* a slot is present iff 1 <= len <= SIG_MAX; never Final an empty slot */
    have1 = (sig1_len >= 1 && sig1_len <= NGX_HTTP_WAF_GEO_SIG_MAX);
    have2 = (sig2_len >= 1 && sig2_len <= NGX_HTTP_WAF_GEO_SIG_MAX);
    ...
    bio = BIO_new_mem_buf(ngx_http_waf_geo_signing_key, -1);
    if (bio == NULL) {
        goto failed;
    }
     ...
     * libloc hashes it.
     */
    ngx_memcpy(hdr, base + 8, NGX_HTTP_WAF_GEO_HDR_SIZE);
    ngx_memzero(hdr + NGX_HTTP_WAF_GEO_ZERO_BEG,
                NGX_HTTP_WAF_GEO_ZERO_END - NGX_HTTP_WAF_GEO_ZERO_BEG);

    if (EVP_DigestVerifyUpdate(bctx, base, 8) != 1
        || EVP_DigestVerifyUpdate(bctx, hdr, NGX_HTTP_WAF_GEO_HDR_SIZE) != 1
        || EVP_DigestVerifyUpdate(bctx, base + NGX_HTTP_WAF_GEO_DATA_OFF,
                                  size - NGX_HTTP_WAF_GEO_DATA_OFF) != 1)
    {
        goto failed;
    }
    ...
    if (have1) {
        ...
        if (EVP_DigestVerifyFinal(snap, base + NGX_HTTP_WAF_GEO_SIG1_OFF,
                                  sig1_len) == 1)
        {
            valid = 1;
        }
    ...
    if (!valid && have2) {
        ...
        if (EVP_DigestVerifyFinal(snap, base + NGX_HTTP_WAF_GEO_SIG2_OFF,
                                  sig2_len) == 1)
        {
            valid = 1;
        }
...
ngx_http_waf_geo_open(ngx_conf_t *cf, ngx_str_t *path)
{
     ...
     */
    if (size < NGX_HTTP_WAF_GEO_DATA_OFF) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
    ...
    if (size > NGX_HTTP_WAF_GEO_MAX_SIZE) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
     ...
     */
    if (ngx_http_waf_geo_verify(cf, &full, base, size) != NGX_OK) {
        return NULL;
    }
