/*
 * ngx_http_waf_module - lock-free per-IP token-bucket rate limiter
 *
 ...
 * private to waf_rate.c, so this header stays context-neutral (ngx_core only)
 ...
 * for_geo) and passes the parameters; waf_rate.c owns the math and the CAS.
 */

#ifndef _WAF_RATE_H_INCLUDED_
#define _WAF_RATE_H_INCLUDED_


#include <ngx_config.h>
#include <ngx_core.h>

#include "waf_rep.h"   /* ngx_http_waf_verdict_t for the rule selector */


/* fixed-point scale: 1 token == WAF_RATE_SCALE internal units */
#define WAF_RATE_SCALE  1000
 ...
 * as ngx_http_waf_country_add). Stored in an ngx_array_t on the HTTP location
 * conf and the STREAM server conf. ngx_core-only so both heads share it.
 *
 * rate_num_fp / period_ms / burst_fp are pre-validated (see
 * ngx_http_waf_rate_rule_add): rate_num_fp in (0, UINT32_MAX], burst_fp in
 ...
 * zone->data.
 */
ngx_int_t ngx_http_waf_rate_check(void *shm, struct sockaddr *sa,
 ...
 */
ngx_http_waf_rate_rule_t *ngx_http_waf_rate_rule_select(ngx_array_t *rules,
 ...
 */
char *ngx_http_waf_rate_rule_add(ngx_conf_t *cf, ngx_array_t **rules);

/* shm init callback for the waf_rate zone (user-sized; HTTP head installs it) */
ngx_int_t ngx_http_waf_rate_init_zone(ngx_shm_zone_t *shm_zone, void *data);

/* table-saturation drop count (eviction-CAS failures); 0 when shm is NULL */
ngx_atomic_uint_t ngx_http_waf_rate_overflow(void *shm);


#endif /* _WAF_RATE_H_INCLUDED_ */
